...
//输出:相机位姿
//返回:true为成功
bool poseEstimationDirect(const vector<Measurement>& measurent,Mat* gray,Eigen::Matrix3f& K,Eigen::Isometry3d& Tcw);

//g2o中本身没有光度误差的边,需要自己定义一种新的边
//直接法图优化问题是由一个相机位姿顶点与许多条一元边组成的
//如果使用稀疏的直接法,大约会有几百至几千条这样的边
...
protected:
  inline float getPixelValue(float x,float y)
  {
    ...
    float xx = x-floor(x); //floor为向下取整函数
    float yy = y-floor(y);
    return float(
	      (1-xx)*(1-yy)*data[0]+
	      xx*(1-yy)*data[1]+
...
int main(int argc,char **argv)
{
  ...
  srand((unsigned int)time(0)); //随机生成器
  ...
  string rgb_file,depth_file,time_rgb,time_depth; 
   ...
   Mat prev_color;
   //以第一张图像为参考,对后续图像和参考图像做直接法
   for( int index=0;index<10;index++)
   {
     cout<<"************loop"<<index<<"**********"<<endl;
     fin>>time_rgb>>rgb_file>>time_depth>>depth_file;
     color = imread(path_to_dataset+"/"+rgb_file);
     depth = imread(path_to_dataset+"/"+depth_file,-1);   //深度图
	...
	continue;
      ...
      circle(img_show,Point2d(pixel_now(0,0),pixel_prev(1,0)+color.rows),8,Scalar(b,g,r),2);
      line(img_show,Point2d(pixel_prev(0,0),pixel_prev(1,0)),Point2d(pixel_now(0,0),pixel_prev(1,0)+color.rows),Scalar(b,g,r),1);
...
bool poseEstimationDirect(const vector<Measurement>& measurement,Mat* gray,Eigen::Matrix3f& K,Eigen::Isometry3d& Tcw)
{
  //初始化g2o   李代数位姿为6维
  typedef g2o::BlockSolver<g2o::BlockSolverTraits<6,1>> DirectBlock;  //误差是标量,灰度差
  ...
  cout<<"edges in grah: "<<optimizer.edges().size()<<endl;
