...
bool VisualOdometry::addFrame(Frame::Ptr frame)
{
  switch(state_)
  {
    ...
    case OK:
    {
      ...
      {
	//计算当前帧与世界坐标系之间的变换矩阵
	curr_->T_c_w_ = T_c_r_estimated_*ref_->T_c_w_;//T_c_w = T_c_r*T_r*w;
	ref_ = curr_;
	setRef3DPoints();
	num_lost_ = 0;
...
void VisualOdometry::featureMatching()
{
  ...
  float min_dst = std::min_element(matches.begin(),matches.end(),
      ...
      {
	if(m.distance<max<float>(min_dst*match_ratio_,30.0))
	{
